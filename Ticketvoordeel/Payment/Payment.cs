...
namespace Ticketvoordeel.Payment
{
    public class SisowPayment
    {
        ...
        // DirectoryRequest
        public async Task<Tuple<int, bool, string>> DirectoryRequest()
        {
            int ex;
            string select;
            ...
            if (ex < 0)
                return new Tuple<int, bool, string>(ex, false, select);
            for (int i = 0; i < issuerid.Length; i++)
            {
                select += "<option value=\"" + issuerid[i] + "\">" + issuername[i] + "</option>";
            }
            select += "</select>";
            return new Tuple<int, bool, string>(0, true, select);
        }
        ...
        public async Task<long> RefundRequest(string trxid)
        {
            trxId = trxid;
            string sha1 = GetSHA1(trxId + merchantId + merchantKey);
            string[] pars = { "merchantid", merchantId, "trxid", trxId, "sha1", sha1 };
            if (!await send("RefundRequest", pars))
                return -1;
        ...
        public async Task<int> InvoiceRequest(string trxid, params string[] keyvalue)
        {
            trxId = trxid;
            string sha1 = GetSHA1(trxId + merchantId + merchantKey);
            string[] pars = { "merchantid", merchantId, "trxid", trxId, "sha1", sha1 };
            if (!await sendAsync("InvoiceRequest", pars, keyvalue))
                return -1;
            invoiceNo = parse("invoiceno");
        ...
        public async Task<int> CancelReservationRequest(string trxid)
        {
            trxId = trxid;
            string sha1 = GetSHA1(trxId + merchantId + merchantKey);
            string[] pars = { "merchantid", merchantId, "trxid", trxId, "sha1", sha1 };
            if (!await send("CancelReservationRequest", pars))
                return -1;
            status = parse("status");
            if (string.IsNullOrEmpty(status))
            {
        ...
        public async Task<int> CreditInvoiceRequest(string trxid, params string[] keyvalue)
        {
            trxId = trxid;
            string sha1 = GetSHA1(trxId + merchantId + merchantKey);
            string[] pars = { "merchantid", merchantId, "trxid", trxId, "sha1", sha1 };
            if (!await sendAsync("CreditInvoiceRequest", pars, keyvalue))
                return -1;
            invoiceNo = parse("invoiceno");
            if (string.IsNullOrEmpty(invoiceNo))
            {
